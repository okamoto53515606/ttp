...
### 踏み台（多段接続）設定 <sup>v1.1.0 追加</sup>

「☑ 踏み台経由で接続する」にチェックを入れると、踏み台サーバ経由で別のサーバに自動ログインする設定が表示されます。

![接続先の登録・編集画面（踏み台設定）](screenshot/TTPConnectionSettingHop.png)

| 項目 | 説明 | 例 |
|---|---|---|
| **プロンプト1** | 踏み台ログイン後のプロンプト | `$` |
| **コマンド1** | 踏み台で実行するコマンド | `ssh user@192.168.1.10` |
| **プロンプト2 (PW要求)** | ターゲットのパスワードプロンプト | `password:` |
| **ターゲットPW** | ターゲットサーバのパスワード | — |
| **プロンプト3** | ターゲットログイン後のプロンプト | `$` |
| **コマンド3** | ターゲットで実行するコマンド | `sudo systemctl status nginx` |
...
[踏み台にSSH接続]
  → プロンプト1 を待つ
  → コマンド1 を送信（例: ssh user@target）
  → プロンプト2 を待つ（例: password:）
  → ターゲットPW を送信
  → プロンプト3 を待つ
  → コマンド3 を送信
```

> **注意**: 踏み台経由時は接続タイムアウトが自動的に **15秒** に延長されます（通常は5秒）。  
> プロンプト文字列は Tera Term の `wait` コマンドによる部分一致です。`$` や `#` など、画面に表示される文字列の一部を指定してください。  
> **ターゲットPW も踏み台PW と同様に暗号化されて保存されます。**

#### 事前準備: SSH ホスト鍵の確認について

初めて接続するサーバでは、sshコマンドが以下のような確認ダイアログを表示します：

```
Are you sure you want to continue connecting (yes/no)?
```

このダイアログは TTP マクロでは自動処理できません。**事前に 踏み台サーバからsshコマンドで手動ログインし、ホスト鍵を受け入れておいてください。** 一度受け入れれば次回以降は表示されなくなります。

- 踏み台サーバとターゲットサーバの **両方** について、事前に手動ログインしてホスト鍵を登録しておく必要があります
- Tera Term のホスト鍵キャッシュは `TERATERM.INI` の設定に従います（デフォルトではレジストリに保存）
